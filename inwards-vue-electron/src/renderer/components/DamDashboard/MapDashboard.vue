...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getselectedDamSites () {
        let _selectedDamSitesId = [];
        for (let i = 0; i < this.selectedDamSites.length; i++) {
          _selectedDamSitesId.push(this.selectedDamSites[i].split(' ')[0]);
        }
        return _selectedDamSitesId;
      },
      ...
      toggleselectedDamSitesByStationNames (selectedStationNames, unselectedStationNames) {
        let self = this;
        this.stationsVectorLayer.getSource().forEachFeature(function (feature) {
          let station = feature.get(self.keys.station);
          const index = self.selectedDamSites.indexOf(station);
          if (selectedStationNames.indexOf(station) !== -1) {
            ...
            if (index === -1) {
              self.selectedDamSites.push(station);
            }
            ...
            if (index > -1) {
              self.selectedDamSites.splice(index, 1);
            }
          }
        });
      },
