...
created () {
  ...
  this.$axios.get('https://inwards.award.org.za/app_json/iucma_unverified.php?sd=' + startDate + '&ed=' + endDate)
    .then(
      response => {
...
methods: {
  updateTable () {
    ...
    this.$axios.get('https://inwards.award.org.za/app_json/iucma_unverified.php?sd=' + this.formatDate(startDate) + '&ed=' + this.formatDate(endDate))
      .then(
        response => {
          this.dws = response.data;
         // console.log(this.dws);
