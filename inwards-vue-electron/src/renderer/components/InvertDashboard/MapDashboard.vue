  ...
  export default {
    data () {
      return {
        ...
        invertVectorLayer: new VectorLayer({
          source: new VectorSource(),
          style: function (feature) {
            return new Style({
              image: new CircleStyle({
    ...
    mounted () {
      ...
      */
      ...
      this.map.addLayer(this.invertVectorLayer);
    ...
    methods: {
      ...
      loadInvertStationsToMap (stationsGeoJSONData) {
        this.invertVectorLayer.setSource(new VectorSource({
          features: (new GeoJSON({
            defaultDataProjection: 'EPSG:4326'
      ...
      selectInvert (selectedInvert) {
        ...
        if (selectedInvert.length === 0) {
          return;
        }
        let extent = Extent.createEmpty();
        for (let i = 0; i < selectedInvert.length; i++) {
          let catchment = selectedInvert[i];
      ...
      toggleSelectedInvertStationsByStationNames (selectedStationNames, unselectedStationNames) {
        let self = this;
        this.invertVectorLayer.getSource().forEachFeature(function (feature) {
